...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->CFGR |= RCC_CFGR_SW_PLL;               // 0x00000002 -> 0x00000001    168M -> 8M 
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* Select the main PLL as system clock source */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));
    RCC->CFGR |= RCC_CFGR_SW_PLL; // 0x00000002  -> 0x00000001

    /* Wait till the main PLL is used as system clock source */
    
    /*
    while ((RCC->CFGR & (uint32_t)RCC_CFGR_SWS ) != RCC_CFGR_SWS_PLL); // pll °ü·Ã ¹®Àå 
    {
    }
    */
  }
