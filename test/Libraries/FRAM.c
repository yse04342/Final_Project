...
void Fram_Status_Config(void)			// 초기화 2번째 
{
	...
	FRAM_CS_LOW;			// low 해야지 enable 되기 때문에. 
	SPI2_Send(data);
...
void Fram_Write(short addr,unsigned char data)
{
	...
	Data[0] = WRITE;				// 첫 비트가 R or W 이기 때문에 
	Data[1] = addr>>8;
	Data[2] = addr&0xff;			//주소는 address 보내고 
	Data[3] = data;
  
	FRAM_CS_LOW;
	SPI2_Send(Data[0]);				// 4바이트를 보내면 
	SPI2_Send(Data[1]);				// 1과 2 는 주소가 보내고 
	SPI2_Send(Data[2]);
	SPI2_Send(Data[3]);				// 실제 데이터가 들어가 있고 
	FRAM_CS_HIGH;				// 이때 FRAM 에 들어가게 된다. 
}
...
unsigned char Fram_Read(short addr)
{
	...
	SPI2_Send(data[1]);
	SPI2_Send(data[2]);
	read_data = SPI2_Send(0xff);			// 아무거나 보내면 밀려서 데이터를 읽는다. 
	FRAM_CS_HIGH;				// READ 이기 때문에 그렇게 된다. 
