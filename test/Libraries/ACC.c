...
uint8_t SPI1_Send(uint8_t byte)		// 1 byte 데이터를 가속도센서에 전송 
{
        ...
        while ((SPI1->SR & (1<<0)) == RESET);		// SR.RXNE = 1 ?	
...
UINT8 SPI1_GetData(UINT8 *pBuf)
{
	// Get ACC_X
        CLR_ACC_NSS1();  
        SPI1_Send(CMD_READ | CMD_REG_OUT_X_L);	// Master <- Slave(sensor)
                                             	// reading a data from OUT_X (이 때 x값은 2개의 8비트로 이루어져 있기 때문이다.)
                                                // 그렇기 때문에 그 값을 Buf 에 보내기 위해서는 Dummy 를 보내서 받는다. 
        // 주요 포인트 첫번째 보낸 데이터는 쓰레기 2 번째 보내는게 진짜이기 때문에 밑에 문구로 인해 buf[0] 에 제대로 된 값이 들어간다.
        pBuf[0] = SPI1_Send(DUMMY);		// 가속도 X 값은 16비트이므로 두개의 dummy를 보냄
        pBuf[1] = SPI1_Send(DUMMY);                     // dummy 는 쓰레기 값이기 때문에 밀어내기 용으로 사용된다. 
        SET_ACC_NSS1();                 // 통신 안하다가 
        UTIL_DelayUS(1);                        // 쓰기용일 경우에는 Dummy 대신에 실제로 값을 받아 온다. 
...
void SPI1_CalData(UINT8 *pBuf, UINT16 *pTxBuf) 
{
        ...
        int16 TempX0, TempY0, TempZ0 , TempXL;
      ...
      // 여기서 문장들을  + - 을 구분하여 출력할 수 있도록 만들어라 .
   
    
          pTxBuf[0] = (TempX0*0.061/100);               //가속도 센서에서 받아온 값을 2g 로 나타내기 위해서 0.061을 곱하고 범위를 맞추기 위해 100을 나눔 
         pTxBuf[1] = (TempY0*0.061/100);                //가속도 센서에서 받아온 값을 2g 로 나타내기 위해서 0.061을 곱하고 범위를 맞추기 위해 100을 나눔 
        pTxBuf[2] = (TempZ0*0.061/100);                 // 가속도 센서에서 받아온 값을 2g 로 나타내기 위해서 0.061을 곱하고 범위를 맞추기 위해 100을 나눔 
}
